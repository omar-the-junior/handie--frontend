    ...
    <div>
      <div className="py-14">
        {cartItems.length === 0 ? (
          ...
          <div className="max-w-screen-xl mx-auto grid grid-cols-[repeat(auto-fit,_minmax(320px,_1fr))] gap-4">
            {cartItems.map((item) => (
              <WishlistItem
                key={item.id}
...
function WishlistItem({ name, price, imageSrc, onRemove, id }: WishlistItemProps) {
  return (
    <div className="mb-4 flex w-full flex-col items-start rounded-lg p-4">
      <div className="flex flex-col ml-5">
        ...
        <div>
          <button className="btn flex w-full items-center justify-center space-x-3">
            <img src="/images/Group.svg" className="h-6 w-6" alt="Cart Icon" />
            <span>Move to Cart</span>
          </button>

          <button
            className="btn btn-outline mt-10 flex w-full items-center justify-center space-x-3"
            onClick={() => onRemove(id)} 
          >
            <img
              ...
              alt="trash Icon"
            />
            <span className='mr-3 text-charcoal'>Remove</span>
          </button>
        </div>
      </div>
    </div>
